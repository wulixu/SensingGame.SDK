...
namespace SensngGame.ClientSDK
{
    public class GameServiceClient : IGameServiceClient
    {
        ...
        public async Task<string> PostAction2TaoByUser(string mixName,string gameImage, string gameName, int score, ActionStatus actionStatus)
        {
            var absolutePath = $"https://taobao.troncell.com/api/v1/Taobao/PostActionByUser";
            //var absolutePath = $"http://localhost:61084/api/v1/Taobao/PostActionByUser";
            var nameValues = new NameValueCollection();
            nameValues.Add("subKey", _subscriptionKey);
            nameValues.Add("score", score.ToString());
            nameValues.Add("status", actionStatus.ToString());
