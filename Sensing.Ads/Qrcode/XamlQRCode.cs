...
namespace SensingAds.Qrcode
{
    public class XamlQRCode : AbstractQRCode, IDisposable
    {
        ...
        public DrawingImage GetGraphic(Size viewBox, Brush darkBrush, Brush lightBrush, bool drawQuietZones = true)
        {
            ...
            var graphics = drawing.Open();
            graphics.DrawRectangle(lightBrush, null, new Rect(new Point(0, 0), new Size(qrSize, qrSize)));
            ...
            for (int xi = offsetModules; xi < drawableModulesCount + offsetModules; xi++)
            {
                ...
                for (int yi = offsetModules; yi < drawableModulesCount + offsetModules; yi++)
                {
                    if (this.QrCodeData.ModuleMatrix[yi][xi])
                    {
                        ...
                        graphics.DrawRectangle(darkBrush, null, new Rect(x, y, unitsPerModule, unitsPerModule));
                    }
                    y += unitsPerModule;
                }
                x += unitsPerModule;
    ...
    {
        public static DrawingImage GetQRCode(string plainText, int pixelsPerModule, string darkColorHex, string lightColorHex, ECCLevel eccLevel, bool forceUtf8 = false, bool utf8BOM = false, QRCodeGenerator.EciMode eciMode = QRCodeGenerator.EciMode.Default, int requestedVersion = -1, bool drawQuietZones = true)
        {
