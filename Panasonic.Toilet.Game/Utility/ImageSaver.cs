...
namespace TronCell.Game.Utility
{
    ...
    public static class ImageSaver
    {
        ...
        /// <returns>The returned ImageSource of the controlToConvert</returns>
        public static Boolean SaveImage4FrameworkElement(Visual controlToConvert, String fileName,double width, double height)
        {
            try
            {
        ...
        public static Boolean SaveImage4FrameworkElement(Visual controlToConvert, String fileName, double width, double height,int sclae)
        {
            try
            {
                var imageType = GetImageTypeByFileName(fileName);
                ...
                using (var outStream = new FileStream(fileName, FileMode.Create))
                {
                    if (width > 0)
                    {
                        ...
                        using (DrawingContext ctx = dv.RenderOpen())
                        {
                            ...
                            ctx.DrawRectangle(vb, null, new Rect(0, 0, (double)width / 2, (double)height / 2));
                        }
                        RenderTargetBitmap rtb = new RenderTargetBitmap((Int32)width/2, (Int32)height/2, 96.0, 96.0, PixelFormats.Pbgra32);
                        rtb.Render(dv);
                        var encoder = GetBitmapEncoderByImageType(imageType);
